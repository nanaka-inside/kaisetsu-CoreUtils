...
b2sum
-----
coreutils 8.26から追加されたコマンドです。BLAKE2 [#bleack2]_ のハッシュ値を出力します。BLAKE2とは、MD5やSHA-1、SHA-2、SHA-3より早い暗号化ハッシュ関数で、標準SHA-3と同程度の安全性があります [#bleack22]_ [#bleack222]_ 。
...
また8.26から ``--total`` が追加されました。こんな感じです。totalの数字は ``-123`` で取り出せます。
...
GNU MP [#core-factor-gmp]_ を使わずにビルドされたfactorコマンドは、single-precision算術が有効になります。その算術方法は、小さい数字を計算することが得意で、2^64以上の数字はサポートしていません。158909489063877810457や222087527029934481871の素因数分解をするときにループに陥りましたが、coreutils 8.20でバグって、8.27 で修正されました。普通こんなことしないと思いますが念のため。
...
``seq 1 0 10`` ってコマンドを実行すると無限ループしていました。coreutils 8.26で実行するとエラーになります。見逃されていたというかなんというか。
