...
気づけば、Coreutilsのマニュアルを読んで、知らないコマンドやいままで使ったことがないコマンドのオプションを見つけました。
...
また、筆者が検証した環境は さくらのVPS CentOS 5.7 と amazon EC2 のマイクロインスタンスです。
...
まさに作っているところで、初心者向けに基本的な概念は説明してないよ！ [#tukkomi]_ 興味があれば改善していってね。
...
``chown`` や ``chgrp`` は、変更するユーザ名やグループ名を引数に与えると指定したユーザ名やグループ名を変更することが出来ます。
...
[#chgrp]_ Coreutilsのマニュアル2.6章に書いてあります。man引いても出てこないですね。ちなみに ``+`` はユーザ名やグループ名に使えません。実際に実行してみると「useradd: invalid user name 'hoge+'」だそうです。そして、Solaris 10なら+記号が使えるようです。
