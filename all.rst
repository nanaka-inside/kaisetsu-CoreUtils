...
なぜ今、Coreutilsなのか。linuxのコンソールでお仕事をしていると、シェルスクリプトで楽をしようとします。
そのとき、コマンドを調べていたら、Coreutilsのマニュアルにたどり着きました。
気づけばCoreutilsのマニュアルを読んで、知らないコマンドや、いままで使ったことがないコマンドのオプションを発見しました。
blogなどのエントリだと、コマンド単発での解説に始終してしまいがちです。
Coreutilsのマニュアルを上から下まで読んでいって、ひと通りコマンドをオプションを知ると、シェルスクリプトの世界が広がります。
...
.. [#core-yousei] どこぞの妖精さんをお借りしました。まったく扱いにくいキャラクターだぜ。文章にするとかどくせいがひじょうにわるいです？

普段linuxを使っているけど、このコマンド知らなかった、こんな使い方ができるのか、という発見が一つでもあれば幸いです。筆者がよく使うコマンドや Tips も載せました。それでは、深遠なるCoreutilsの世界へようこそ。
...
「コアユーティルズ」と読みます。普段、読み方を気にしていませんでしたが、あたらめて書いてみるとダサいですね。まあ、そのへんは置いといて、Coreutilsとは、
``ls`` や ``cat`` など、linuxでは欠かせないコマンドをまとめたパッケージです。Coreutilsが登場する前は、Fileutils, Shellutils, Textutilsというutilsがありました。Coreutilsは、それらを統合したものです。ChangeLogをみたところ、一番古い日付は2002-07-01でした。おそらくそのころに統合されたのでしょう。メジャーバージョンは2003年4月にバージョン5として登場しました。
2014年4月現在、8.22がリリースされています。
...
第1章はCoreutilsの紹介から始まっています。少し読んでみましょう。
Coreutilsのマニュアルは、未確認で進行形、じゃなかった、作成途中です。
まさに作っているところで、初心者向けに基本的な概念は説明してないよ！ [#tukkomi]_ 興味があれば改善していってね。あと、バグを見つけたら ``bug-coreutils@gnu.org`` に送ってね。そのとき、再現環境も一緒に送ってね！ [#senjin-namae]_ だそうです。

.. [#tukkomi] この本で適宜、追記します
...
.. [#core-sec] Coreutilsのセクション3「Output of entire files」です
